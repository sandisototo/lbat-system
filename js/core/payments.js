...
.controller("PaymentsController", ['$http', '$scope', '$window', 'SharedProperties', 'toastr', 'MainFactory', function($http, $scope,$window, SharedProperties, toastr, mainFactory){
  ...
  $scope.get_all_users()
  // Paid selected Paid user_ids
 $scope.selectedAsPaidUsers = [];

 // Toggle selection for a given user by id
 $scope.togglePaidSelection = (user) => {
   let user_index = $scope.selectedAsPaidUsers.indexOf(user);

   // Is currently selected
   if (user_index > -1) {
     $scope.selectedAsPaidUsers.splice(user_index, 1)
   }else {
     $scope.selectedAsPaidUsers.push(user)
   }
   console.log('selectedAsPaidUsers --', $scope.selectedAsPaidUsers)
 }
